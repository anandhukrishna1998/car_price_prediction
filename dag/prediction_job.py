...
def check_for_new_data(ti):
    ...
    new_files = [f for f in files if datetime.fromtimestamp(os.path.getmtime(f)) > datetime.fromisoformat(last_processed_time)]
...
def make_predictions_fun(**kwargs):
    ...
    for file_path in new_files:
        ...
        headers = {'Content-Type': 'application/json'}  # This header is technically optional here as requests infers it
        
        response = requests.post(api_url, data=json.dumps({"data": new_data_df_data}), headers=headers)
